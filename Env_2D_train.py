...
class EnvUAV_train():
    ...
    def forward(self, action):
        ...
        reward_distance = -np.linalg.norm(self.position-self.target,axis=1)
        ...
        reward_mutual = 0
        reward_distance = reward_distance * 0.01
        reward_done = 100 if done2 else 0
        print(" distance:",reward_distance)
        #self.reward = reward_sparse + reward_barrier + reward_distance + reward_action + reward_mutual + reward_agent
        self.reward = reward_distance + reward_action + reward_done
        #################################################################################################  
