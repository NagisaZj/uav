...
class Net(nn.Module):
    def __init__(self, s_dim, a_dim):
        ...
        self.mu1 = nn.Linear(100, a_dim)
        self.sigma1 = nn.Linear(100, a_dim)
    ...
    def choose_action(self, s):
        ...
        m1,m2 = self.distribution(mu1.view(1, ).data,
                                  sigma1.view(1, ).data),self.distribution(mu2.view(1, ).data, sigma2.view(1, ).data)
    ...
    def run(self):
        ...
        while self.g_ep.value < MAX_EP:
            ...
            for t in range(MAX_EP_STEP):
                ...
                s_, r, done, done2 = self.env.forward([a1.clip(-2, 2),a1.clip(-1, 1)])
                if t == MAX_EP_STEP - 1:
                    done = True
                ep_r += r
                buffer_a.append(a)
                ...
                if total_step % UPDATE_GLOBAL_ITER == 0 or done or done2:  # update global and assign to local net
                    # sync
                    push_and_pull(self.opt, self.lnet, self.gnet, done, s_, buffer_s, buffer_a, buffer_r, GAMMA)
                    buffer_s, buffer_a, buffer_r = [], [], []

                    if done or done2:  # done and print information
